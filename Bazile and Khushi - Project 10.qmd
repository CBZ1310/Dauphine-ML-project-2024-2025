...
The table below describe the percentage of missing value by variable in the learning set. The value of the main dataset has a 100% completion rate.
...
na_percentage_by_act <- learn_merged_uncoded %>%
  ...
  arrange(desc(Remuneration))  # Trié par le pourcentage de NA pour "Remuneration"


kable(na_percentage_by_act, caption = "Percentage of Missing Values by Activity Status")

```

More precisely, as we can see below, in average 20% of the employment data for Active workers are missing in several variables.
...
We try to go further in the analysis by identifying if using the variable "JOB_42" we found some logical pattern.
