...
We have 3 binary variables, studyig target and sex. We choose to transform sex into a binary variable
...
learn_merged <- learn_merged %>%
  mutate(Studying = ifelse(Studying == "TRUE", 1, 0))
```

We also decided at this step of our work to transform the string values into factor.
...
2.  For work-related variable :
    ...
    -   Fill the missing values with "Not applicable" for all non-numerical variables and"-1"
        ...
        -   and, for active workers woreare self employed
...
# Exclude "target" and "sports" columns from imputation
columns_to_impute <- setdiff(colnames(learn_merged), c("target", "sports"))

# Prepare the dataset for imputation (convert character columns to factors)
learn_merged <- learn_merged %>%
  select(all_of(columns_to_impute)) %>%
  mutate(across(where(is.character), as.factor),  # Convert character columns to factors
    across(where(is.logical), as.factor)     # Convert logical columns to factors
  )
...
```{r}
# Créer une recette en précisant que `target` est la variable cible
knn_recipe <- recipe(target ~ ., data = learn_merged) %>%
  step_impute_knn(all_predictors(), neighbors = 5)  # Appliquer KNN aux prédicteurs uniquement

# Préparer et appliquer la recette
knn_prep <- prep(knn_recipe, training = learn_merged)

# Imputer les valeurs manquantes
learn_merged_imputed <- bake(knn_prep, new_data = NULL)

# Vérifier les résultats
summary(learn_merged_imputed)
...
my_grid <- expand.grid(
  ...
  min_n = c(2L, 7L, 12L, 24L, 48L, 84L, 100L),                
  tree_depth = c(2L, 3L, 4L, 5L, 7L, 10L, 15L))

## Re-sampling
set.seed(12)
compound_v <- vfold_cv(DATA_A_AJOUTER, v = 5, strata = target)

## Using tune_grid on the tree


X <- setdiff(names(DATA_A_AJOUTER), c("target", "sport"))

model_formula <- as.formula(paste("target ~", paste(X, collapse = " + ")))

my_tuned_tree <- my_tree %>% 
...
```{r}
my_final_tree <- my_tree %>% 
  finalize_model(my_best_parameters)

final_fit <- my_final_tree %>% last_fit(model_formula, split = splitted_data)
```
...
my_grid_rf <- expand.grid(
  ...
  min_n = c(2L, 7L, 12L, 24L, 36L, 48L),
  trees = c(100L, 200L,300L))

# Cv fold
homelander <- vfold_cv(DATA_A_AJOUTER, v = 5, strata = target)
...
kable(best_rf_params, caption = "Table 4. Parameters of the random forest model")
```

```{r}
# Final model
final_amazonia_model <- amazonia %>%
  finalize_model(best_rf_params)

amazonia_fit <- final_amazonia_model %>%
  fit(model_formula, data = train_data_rf)
```
...
train_predictions_rf <- amazonia_fit %>%
  predict(new_data = train_data_rf) %>%
  bind_cols(train_data_rf %>% select(target))
