...
```{r table_on_the_type_of_data_in_the_dataset}
...
```{r list_of_string_var_and_their_unique_values}
...
```{r selection_of_data_on_town}
#Data keep for the maps and addition of X and Y coordinates
learn_map <- learn_merged 
city_loc <- read_csv("Data - source/project-10-files/city_loc.csv", show_col_types = FALSE)

learn_map <- learn_map %>% 
  left_join(city_loc, by = "INSEE") 

#Suppression of unecessary data
learn_merged <- learn_merged %>%
...
```{r import_of_dictionnary_on_job_decs}
...
```{r transformation_of_string_in_factor}
learn_merged <- learn_merged %>%
...
```{r Distribution_of_the_target_variable}
...
```{r distribution_of_the_target_variable_in_percent}
## Percent (%)
...
```{r map_dep_part_preparation}

points_sf <- st_as_sf(learn_map, coords = c("long", "Lat"), crs = 4326, remove = FALSE)
```

#### Renumeration

```{r map_dep_on_renumeration}
# Summarize mean remuneration by DEP
aggregated_data <- learn_map |>
  group_by(DEP) |>
...
aggregated_data <- aggregated_data |>
  left_join(learn_map |> select(DEP, Lat, long), by = "DEP") |>
  distinct()
...
```{r map_dep_on_retirement_pay}
aggregated_retirement_pay <- learn_map |>
  group_by(DEP) |>
...
Using the library "DateExplorer", we create an alternative representation with the percentage of missing values looking only variable with at least one missing value.

```{r graph_on_na_with_only_column_with_na}
...
learn_merged <- learn_merged %>%
  mutate(Studying = as.factor(Studying))
...
```{r}
# Transform 'Studying' into a factor
learn_merged <- learn_merged %>%
  mutate(Studying = as.factor(Studying))

# Lasagna with salmon : A special recipe for logit
lasagna_for_logit <- recipe(target ~ ., data = learn_merged) %>%
  update_role(PRIMARY_KEY, new_role = "ID") %>%
  step_impute_median(all_numeric_predictors()) %>%
  step_impute_mode(all_nominal_predictors()) %>% 
...
#  Creation of the logit Model
logit_m <- logistic_reg(penalty = tune(), mixture = tune()) %>%
  set_engine("glmnet") %>%
  set_mode("classification")

# Workflow
logit_workflow <- workflow() %>%
  add_recipe(lasagna_for_logit) %>%
  add_model(logit_m)
...
logit_grid <- grid_regular(
  penalty(range = c(0.001, 0.1)),    # Penalty values
  mixture(range = c(0, 1)),          # Elastic net mixing values
  levels = 10                        # Number of grid points
)

# Tuning
set.seed(12)
logit_result <- tune_grid(
  logit_workflow,
...
# Results
collect_metrics(logit_result)

# Best Parameters
best_params <- select_best(logit_result, metric = "roc_auc")

# Final Workflow with Best Parameters
final_logit_workflow <- finalize_workflow(logit_workflow, best_params)

# Fit the Final Logistic Regression Model
final_logit_model <- fit(final_logit_workflow, data = learn_merged)
```

```{r}
# Prediction
train_predictions <- final_logit_model %>%
  predict(new_data = learn_merged, type = "prob") %>%
  bind_cols(final_logit_model %>%
              predict(new_data = learn_merged, type = "class")) %>%
  bind_cols(learn_merged %>% select(target))

# Metrics
train_metrics <- train_predictions %>%
  metrics(truth = target, estimate = .pred_class, .pred_A)
...
my_grid <- expand.grid(
  ...
  tree_depth = c(1L, 5L, 10L, 15L, 30L, 50L, 100L))
...
Our final tree using the ROC auc as key metric has the following parameters :

```{r}
my_best_parameters <- my_tuned_tree %>% select_best(metric = "roc_auc")
kable(my_best_parameters, caption = "Best Parameters for the Decision Tree Model")
```

```{r}
my_final_tree_workflow <-  finalize_workflow(tree_workflow ,my_best_parameters)

final_fit <- fit(my_final_tree_workflow, data = learn_merged)
...
final_predictions <- final_fit %>%
  predict(new_data = learn_merged, type = "prob") %>%  # Predicted probabilities
  bind_cols(predict(final_fit, new_data = learn_merged, type = "class")) %>%  # Predicted classes
  bind_cols(learn_merged %>% select(target))  # True target values

#  Metrics
metrics_result <- final_predictions %>%
  metrics(truth = target, estimate = .pred_class, .pred_A) %>%
  bind_rows(
    final_predictions %>%
      roc_auc(truth = target, .pred_A) %>%
      mutate(.metric = "roc_auc"),
    final_predictions %>%
      precision(truth = target, estimate = .pred_class) %>%
      mutate(.metric = "precision"),
    final_predictions %>%
      recall(truth = target, estimate = .pred_class) %>%
...
metrics_result %>%
  kable(
    caption = "Performance Metrics for the Decision Tree Model",
...
autoplot(train_conf_matrix, type = "heatmap") +
...
my_grid_rf <- expand.grid(
  mtry = c(1L, 5L, 10L, 15L, 30L, 50L, 100L), 
