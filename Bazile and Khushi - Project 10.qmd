...
na_percentage_by_act_value <- learn_merged_uncoded %>%
  ...
  arrange(desc(LAST_JOB_42))  # Trier par la colonne LAST_JOB_42

# Affichage du tableau formaté
kable(na_percentage_by_act_value, caption = "Percentage of Missing Values by Column for Each Activity Status")
...
3.  For retirement related-variable :
    ...
    -   let the NA for non-retired individual because it corresponds to natural missing values

    -   Impute data for LAST_Job42, last_emp_type and retirement_age using modal value

    -   Impyte the data for retirement_pay using

```{r}
distri_ret_pay <- ggplot(retirees_data, aes(x = Retirement_pay)) +
