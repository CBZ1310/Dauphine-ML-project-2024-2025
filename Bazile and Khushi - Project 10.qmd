...
```{r}
# Calcul du total de NA pour chaque colonne
total_na_per_column <- learn_merged_uncoded %>%
  summarise(
...
```{r}
retired_data <- learn_merged_uncoded %>%
  filter(JOB_42_translated == "Retired workers")

summary(retired_data$remuneration)

ggplot(learn_merged, aes(x = remuneration)) +
  geom_histogram(bins = 30, fill = "skyblue", color = "black") +
  labs(
    title = "Distribution de la rémunération des Retired Workers",
    x = "Rémunération",
    y = "Fréquence"
  ) 
```
