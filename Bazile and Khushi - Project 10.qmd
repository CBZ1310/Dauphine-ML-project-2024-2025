...
```{r graph_distribution_target}

ggplot(learn_merged, aes(x = as.factor(target))) +
  geom_bar(fill = "darkred", color = "black") +
  labs(title = "Distribution of the target variable", x = "Target", y = "N") +
  theme_minimal()
...
2.  For work-related variable :
    ...
    -   Fill the missing values with "Not applicable" for all non-numerical variables and"-1" for numerical values for the active status that are not either retiree or active worker
...
learn_merged[, columns_to_impute] <- completed_data_rf
